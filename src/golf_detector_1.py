...
class Test(Dataset):
    def __init__(self, test_dir, phase=None):
    ...
    def __getitem__(self, index):
        ...
        image = io.imread(test_dir + image_name)
        image = transform[self.phase](image)
...
test_dataset = Test(test_dir,phase='test')
...
def predict(model, data_path, param, batchsize=64, gpu=True, threshold=0.5):
    ...
    with torch.no_grad():
        model.load_state_dict(torch.load(param, map_location=torch.device('cpu')))
        model = model.eval()
        image = io.imread('/Users/user/Project/マルティスープ/compe_golf/test/test_0.tif')
        image = transform['test'](image)
        pred = nn.Softmax(model(image),dim=1)
        #pred = np.argmax(pred)
        print(pred)
'''
...
def predict(model, data_path, param, batchsize=64, gpu=True, threshold=0.5):
    ...
    image = io.imread('/Users/user/Project/マルティスープ/compe_golf/test/test_1.tif')
    image = transform['test'](image)
...
def main(train_flag,predict_flag):
    ...
    if train_flag:
        ...
        predict(model=model, data_path=test_dir, param=param_path)
