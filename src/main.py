...
x_train, x_val, y_train, y_val = train_test_split(image_name_list, label_list, test_size=0.2, stratify=label_list, random_state=42)
...
class Brightness():
    def __call__(self, image):
        ...
        if p <= 0.25:
            ...
            image_transformed = exposure.adjust_gamma(image, gamma=1.2)
            return image_transformed
        elif 0.5 < p <= 0.75:
            image_transformed = exposure.adjust_gamma(image, gamma=1.4)
            return image_transformed
        else:
            image_transformed = exposure.adjust_gamma(image, gamma=1.6)
            return image_transformed
...
class Contrast():
    def __call__(self, image):
        ...
        if p <= 0.25:
            ...
            image_transformed = exposure.adjust_sigmoid(image, cutoff=0.5, gain=2.0)
            return image_transformed
        elif 0.5 < p <= 0.75:
            image_transformed = exposure.adjust_sigmoid(image, cutoff=0, gain=2.0)
            return image_transformed
        else:
            image_transformed = exposure.adjust_sigmoid(image, cutoff=0, gain=6.0)
            return image_transformed
'''
...
class Brightness():
    def __call__(self, image):
        p = random.random()
        alpha=0.0#コントラスト0.0で固定
        if p <= 0.25:
            return image
        elif 0.25 < p <= 0.5:
            dst = alpha * image + 25.0
            return np.clip(dst, 0, 255).astype(np.uint8)
        elif 0.5 < p <= 0.75:
            dst = alpha * image + 50.0
            return np.clip(dst, 0, 255).astype(np.uint8)
        else:
            dst = alpha * image + 100.0
            return np.clip(dst, 0, 255).astype(np.uint8)
...
def main(train_flag,predict_flag):
    ...
    if train_flag:
        ...
        param_path = '/Users/user/Project/マルティスープ/compe_golf/src/Epoch13_iou_0.7915.pth'
